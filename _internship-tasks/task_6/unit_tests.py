...
def assert_dataframe_equal(df1: DataFrame, df2: DataFrame):
        # Custom function to compare two Spark DataFrames for equality.
        assert df1.schema == df2.schema, "Schemas do not match"
        assert sorted(df1.collect()) == sorted(df2.collect()), "DataFrames content does not match"
...
def test_case_1():
    ...
    input_df = spark.createDataFrame(input, input_schema)

    # Act
    actual_df = etl(spark, input_df)
